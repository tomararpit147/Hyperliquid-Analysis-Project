...
1. **Loaded & audited** both datasets: 0 missing values, 0 duplicates in either
2. **Parsed timestamps**: `Timestamp IST` format `dd-mm-yyyy HH:MM` → date string, aligned with FG date keys
3. **Merged on date**: 211,218 matched rows across 479 trading days
4. **Derived metrics**:
...
- ⚠️ _Contrarian observation_: "greedy" days are NOT the most profitable for these traders
...
- **Top features** (by importance):
  1. `win_rate` (0.68) — day-level trade accuracy dominates
  2. `fear_greed_value` (0.069) — sentiment index is 2nd most important
  3. `trade_count` (0.063)
  4. `avg_open_size` (0.062)
